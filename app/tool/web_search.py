    ...
    async def execute(self, query: str) -> ToolResult:  # Changed.
        ...
        try:
            ...
            if not organic_results:
                return ToolResult(output="No organic search results found.", error=None)
            # If we got here, we have successful results, we need to return those
            result_links = [
                result["link"] for result in organic_results
            ]  # extract just the links
            return ToolResult(output=result_links, error=None)  # return the links
